...
void setLeds(Colors color) {
  ...
  if (ledmode123 == BOTH) {
    ...
    if (color == BLACK)
      strip.setLedColorData(ledmode123-1, 0, 0, 0);
    else if (color == WHITE)
      strip.setLedColorData(ledmode123-1, 255, 255, 255);
    else 
      strip.setLedColorData(ledmode123-1, strip.hsv2rgb(color*30, 100, 100)); 
  }
  strip.show();   
